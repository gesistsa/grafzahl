...
The distil mBERT is used in this case to model the two languages simultanesouly, whereas the original paper modeled the two languages seperately.

Also, we can see here that the quanteda `corpus` objects can be combined by `+`. In this example, as there is only `docvar` in the corpus ("icom"), that docvar is used as the label to be predicted.
